...
namespace colors {
    ...
    namespace {
        ...
        struct not_equal_converter {
            ...
            static T run(T value) {
                typedef typename std::conditional <
                    SrcBPC<DstBPC, less_converter, greater_conveter>::type conv_type;
        ...
        struct converter {
            template <uint8_t SrcBPC, uint8_t DstBPC, typename T>
            static T run(T value) {
                typedef typename std::conditional<SrcBPC == DstBPC, equal_converter,
                                                  not_equal_converter>::type conv_type;
                return conv_type::template run<SrcBPC, DstBPC>(value);
            }
        };
        ...
        struct element_traits<uint8_t, PixelTraits> {
            ...
            static typename PixelTraits::channel_type to_channel_value(value_type value) {
                return static_cast<typename PixelTraits::channel_type>(
                    converter::run<SrcBPC, DstBPC>(static_cast<pixel_type>(value)));
            }
        ...
        struct element_traits<double, PixelTraits> {
            ...
            template <uint8_t SrcBPC, uint8_t DstBPC>
            static value_type from_channel_value(pixel_type value) {
                return static_cast<value_type>(static_cast<value_type>(value) /
                                               static_cast<value_type>((1 << SrcBPC) - 1));
            }
        };
    ...
    struct basic_color {
        ...
        typename PixelTraits::pixel_type value() const { return this->traits_value<PixelTraits>(); }
        ...
        static basic_color create_from_value(typename ST::pixel_type value) {
            ...
            return basic_color(
                (element_traits_type::template from_channel_value<ST::bits_a, PixelTraits::bits_a>(
                    (value & ST::mask_a) >> ST::shift_a)),
                (element_traits_type::template from_channel_value<ST::bits_r, PixelTraits::bits_r>(
                    (value & ST::mask_r) >> ST::shift_r)),
                (element_traits_type::template from_channel_value<ST::bits_g, PixelTraits::bits_g>(
                    (value & ST::mask_g) >> ST::shift_g)),
                (element_traits_type::template from_channel_value<ST::bits_b, PixelTraits::bits_b>(
                    (value & ST::mask_b) >> ST::shift_b)));
        }
        ...
        typename DT::pixel_type traits_value() const {
            ...
            dest_pixel_type value =
                (((static_cast<dest_pixel_type>(element_traits_type::template to_channel_value<
                                                PixelTraits::bits_a, DT::bits_a>(a))
                   << DT::shift_a) &
                  DT::mask_a) |
                 ((static_cast<dest_pixel_type>(element_traits_type::template to_channel_value<
                                                PixelTraits::bits_r, DT::bits_r>(r))
                   << DT::shift_r) &
                  DT::mask_r) |
                 ((static_cast<dest_pixel_type>(element_traits_type::template to_channel_value<
                                                PixelTraits::bits_g, DT::bits_g>(g))
                   << DT::shift_g) &
                  DT::mask_g) |
                 ((static_cast<dest_pixel_type>(element_traits_type::template to_channel_value<
                                                PixelTraits::bits_b, DT::bits_b>(b))
                   << DT::shift_b) &
                  DT::mask_b));
            return value;
        }
    ...
    template <typename T, typename PixelTraits>
    bool operator==(const basic_color<T, PixelTraits> &lhs,
                    const basic_color<T, PixelTraits> &rhs) {
    ...
    template <typename T, typename PixelTraits>
    basic_color<T, PixelTraits> operator+(const basic_color<T, PixelTraits> &lhs,
                                          const basic_color<T, PixelTraits> &rhs) {
    ...
    inline T &get(basic_color<T, PixelTraits> &c) {
        ...
        pair_type arr[PixelTraits::size] = {pair_type(shift_a, &c.a), pair_type(shift_r, &c.r),
                                            pair_type(shift_g, &c.g), pair_type(shift_b, &c.b)};
        std::sort(arr, arr + PixelTraits::size,
                  [](const pair_type &lhs, const pair_type &rhs) -> bool {
