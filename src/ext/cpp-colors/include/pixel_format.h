...
namespace colors {
    ...
    namespace pixel_format {
        enum pixel_format {
            ...
            indexed1, // 1-bit Indexed. Specifies that the pixel format is 1 bit per pixel and that
                      // it uses indexed color. The color table therefore has two colors in it.
            indexed4, // 4-bit Indexed. Specifies that the format is 4 bits per pixel, indexed.
            indexed8, // 8-bit Indexed. Specifies that the format is 8 bits per pixel, indexed. The
                      // color table therefore has 256 colors in it.
            gray16,   // 16-bit Grayscale. The pixel format is 16 bits per pixel. The color
                      // information specifies 65536 shades of gray.

            bgr24, // 24-bit RGB pixel format with 8 bits per channel. r8g8b8
            color,
            /* bgra32 */ // 32-bit ARGB pixel format with alpha, using 8 bits per channel. a8r8g8b8
            bgr32,  // 32-bit RGB pixel format, where 8 bits are reserved for each color. x8r8g8b8
            bgr565, // 16-bit RGB pixel format with 5 bits for red, 6 bits for green, and 5 bits for
                    // blue.           r5g6b5
            bgr555, // 16-bit pixel format where 5 bits are reserved for each color. x1r5g5b5
            bgra5551, // 16-bit pixel format where 5 bits are reserved for each color and 1 bit is
                      // reserved for alpha. a1r5g5b5
            ...
            bgrap32, // PARGB32. Specifies that the format is 32 bits per pixel; 8 bits each are
                     // used for the alpha, red, green, and blue components. The red, green, and
                     // blue components are premultiplied, according to the alpha component.
            bgrap64, // PARGB64. Specifies that the format is 64 bits per pixel; 16 bits each are
                     // used for the alpha, red, green, and blue components. The red, green, and
                     // blue components are premultiplied according to the alpha component.

            bgr48,  // 48-bit RGB. Specifies that the format is 48 bits per pixel; 16 bits each are
                    // used for the red, green, and blue components.
            bgra64, // 64-bit ARGB. Specifies that the format is 64 bits per pixel; 16 bits each are
