...
namespace colors {
    ...
    namespace {
        ...
        struct symbols<char> {
            ...
            static const char to_lower(char ch) { return static_cast<char>(std::tolower(ch)); }
        };
        ...
        struct symbols<wchar_t> {
            ...
            static const wchar_t to_lower(wchar_t ch) { return static_cast<wchar_t>(towlower(ch)); }
        };
        ...
        struct to_argb_str_dispatch<CharT, T, PixelTraits, false> {
            ...
            operator string_type() {
                ...
                oss << symbols<CharT>::argb_tag() << symbols<CharT>::round_bracket_open
                    ...
                    << static_cast<uint32_t>(c.b) << symbols<CharT>::round_bracket_close;
        ...
        struct to_argb_str_dispatch<CharT, T, PixelTraits, true> {
            ...
            operator string_type() {
                return to_argb_str_dispatch<CharT, typename color_type::value_type,
                                            typename color_type::pixel_traits_type, false>(c);
            }
        };
        ...
        struct to_rgb_str_dispatch<CharT, T, PixelTraits, true> {
            ...
            operator string_type() {
                return to_rgb_str_dispatch<CharT, typename color_type::value_type,
                                           typename color_type::pixel_traits_type, false>(c);
            }
        };
        ...
        struct to_ahex_str_dispatch<CharT, T, PixelTraits, false> {
            ...
            operator string_type() {
                ...
                oss << symbols<CharT>::sharp << std::hex << std::setw(2)
                    << std::setfill(symbols<CharT>::zero) << static_cast<uint32_t>(c.a)
                    << std::setw(2) << std::setfill(symbols<CharT>::zero)
                    << static_cast<uint32_t>(c.r) << std::setw(2)
                    << std::setfill(symbols<CharT>::zero) << static_cast<uint32_t>(c.g)
                    << std::setw(2) << std::setfill(symbols<CharT>::zero)
        ...
        struct to_ahex_str_dispatch<CharT, T, PixelTraits, true> {
            ...
            operator string_type() {
                return to_ahex_str_dispatch<CharT, typename color_type::value_type,
                                            typename color_type::pixel_traits_type, false>(c);
            }
        };
        ...
        struct to_hex_str_dispatch<CharT, T, PixelTraits, false> {
            ...
            operator string_type() {
                ...
                oss << symbols<CharT>::sharp << std::hex << std::setw(2)
                    << std::setfill(symbols<CharT>::zero) << static_cast<uint32_t>(c.r)
                    << std::setw(2) << std::setfill(symbols<CharT>::zero)
                    << static_cast<uint32_t>(c.g) << std::setw(2)
                    << std::setfill(symbols<CharT>::zero) << static_cast<uint32_t>(c.b);
        ...
        struct to_hex_str_dispatch<CharT, T, PixelTraits, true> {
            ...
            operator string_type() {
                return to_hex_str_dispatch<CharT, typename color_type::value_type,
                                           typename color_type::pixel_traits_type, false>(c);
            }
        };

        template <typename T, typename PixelTraits>
        void assign_color_value(basic_color<T, PixelTraits> &c, size_t idx,
                                typename basic_color<T, PixelTraits>::channel_type value) {
    ...
    // Write named color
    template <typename ColorMapper, typename CharT, typename CharTraits, typename T,
              typename PixelTraits>
    inline std::basic_ostream<CharT, CharTraits> &
    write_named_color(std::basic_ostream<CharT, CharTraits> &os,
                      const basic_color<T, PixelTraits> &c) {
    ...
    template <typename ColorMapper, typename CharT, typename CharTraits, typename T,
              typename PixelTraits>
    inline std::basic_istream<CharT, CharTraits> &
    read_named_color(std::basic_istream<CharT, CharTraits> &is, basic_color<T, PixelTraits> &c) {
    ...
    inline std::basic_string<CharT> to_hex_str(const basic_color<T, PixelTraits> &c) {
        typedef basic_color<T, PixelTraits> color_type;
        return to_hex_str_dispatch<CharT, T, PixelTraits,
                                   std::is_floating_point<typename color_type::value_type>::value>(
            c);
    }
    ...
    inline std::basic_string<CharT> to_ahex_str(const basic_color<T, PixelTraits> &c) {
        typedef basic_color<T, PixelTraits> color_type;
        return to_ahex_str_dispatch<CharT, T, PixelTraits,
                                    std::is_floating_point<typename color_type::value_type>::value>(
            c);
    }
    ...
    inline std::basic_string<CharT> to_rgb_str(const basic_color<T, PixelTraits> &c) {
        typedef basic_color<T, PixelTraits> color_type;
        return to_rgb_str_dispatch<CharT, T, PixelTraits,
                                   std::is_floating_point<typename color_type::value_type>::value>(
            c);
    }
    ...
    inline std::basic_string<CharT> to_argb_str(const basic_color<T, PixelTraits> &c) {
        typedef basic_color<T, PixelTraits> color_type;
        return to_argb_str_dispatch<CharT, T, PixelTraits,
                                    std::is_floating_point<typename color_type::value_type>::value>(
            c);
    }
    ...
    inline std::basic_ostream<CharT, CharTraits> &
    operator<<(std::basic_ostream<CharT, CharTraits> &os, const basic_color<T, PixelTraits> &c) {
        ...
        if (is >> ch && ch != symbols<CharT>::sharp) {
            ...
            for (size_t i = 0; i != data_len; ++i) {
                ...
                for (size_t j = 0; j != hex_len; ++j) {
                    if (is >> ch &&
                        ((ch <= symbols<CharT>::number_9 && symbols<CharT>::number_0 <= ch) ||
                        ...
                        if (is >> ch && ch != symbols<CharT>::round_bracket_open) {
                            ...
                            for (size_t i = 0; i != data_len; ++i) {
                                ...
                                if (is >> std::ws >> chv >> ch && ch != symbols<CharT>::comma) {
                                    is.putback(ch);
                                    if (i < data_len - 1) is.setstate(std::ios_base::failbit);
                ...
                if (is >> ch && ch != symbols<CharT>::b) {
                    ...
                    if (is >> ch && ch != symbols<CharT>::round_bracket_open) {
                        ...
                        for (size_t i = 0; i != data_len; ++i) {
                            ...
                            assign_color_value(c, i + 1,
                                               static_cast<channel_type>(chv)); // i==0 --- Alpha
    ...
    template <typename CharT, typename CharTraits, typename T, typename PixelTraits>
    inline std::basic_istream<CharT, CharTraits> &
    operator>>(std::basic_istream<CharT, CharTraits> &is, basic_color<T, PixelTraits> &c) {
        typedef basic_color<T, PixelTraits> color_type;
        typedef typename color_type::channel_type channel_type;
        typedef std::basic_stringstream<CharT, CharTraits, std::allocator<CharT>> stringstream_type;
