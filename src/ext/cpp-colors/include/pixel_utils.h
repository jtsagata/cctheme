...
namespace colors {
    ...
    // The stride is the length of a row measured in bytes, rounded up to the nearest DWORD (4
    // bytes). This allows bitmaps with fewer than 32 bits per pixel (bpp) to consume less memory
    // while still providing good performance. You can use the following function to calculate the
    // stride for a given bitmap.
    //
